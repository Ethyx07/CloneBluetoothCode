...
#include  "ServoTimer2.h" //Uses Timer2 over Servo.h for compatibility issues
ServoTimer2 servo1;
...
void setup()  
{
    ...
    servo1.attach(3); //attaches data cable to Pin 3
 pinMode(4, OUTPUT);
 digitalWrite(4, LOW);
    if (DEBUG)
    {
...
void processCommand()
{
    ...
    if      (strcmp ("UpServo",receivedChars) == 0)  { servo1.write(3000);   }
    else if (strcmp ("DownServo",receivedChars) == 0) { servo1.write(1300);    }
    else{
      
    }
    if (strcmp ("LEDON",receivedChars) == 0) { digitalWrite(4, HIGH);    }
    else if (strcmp ("LEDOFF",receivedChars) == 0) { digitalWrite(4, LOW);    }
